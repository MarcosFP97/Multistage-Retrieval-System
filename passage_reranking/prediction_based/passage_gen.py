...
def get_passages_for_dfs(dfs: Union[List[DataFrame], DataFrame],
                         ...
                         query_pred_file: str = 'stance_prediction/gpt-3/predicted_claims_gtp3.txt',
    ...
    queries = pd.read_csv(query_pred_file, names=["topic", "predicted_query"])
    df = pd.merge(df, queries, how="left", on=["topic"])

    df["score_monot5"] = 0.0
    df['passage'] = ""
    print(f"Generating passages for {len(df)} documents...")
    for i, (query, text) in tqdm(enumerate(zip(df['predicted_query'].values, df['text'].values)), total=len(df)):
...
def generate_passages_from_bm25(model_name: str = 'castorini/monot5-base-med-msmarco',
                                ...
                                query_pred_file: str = 'stance_prediction/gpt-3/predicted_claims_gtp3.txt',
                                out_file: str = 'runs/monot5/monot5pred-2022.csv'):
    directory = Path(bm25_runs_dir)
    dfs = [pd.read_csv(file) for file in directory.glob('*.csv')]
    df = get_passages_for_dfs(dfs, model_name, tokenizer_name, query_pred_file, extra_cols=['score'])
